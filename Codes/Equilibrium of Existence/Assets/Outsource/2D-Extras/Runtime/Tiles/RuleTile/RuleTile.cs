...
namespace UnityEngine
{
    ...
    public class RuleTile : TileBase
    {
        ...
        public class TilingRuleOutput
        {
            ...
            /// <summary>
            /// The random scale factor for this Rule.
            /// </summary>
            public float m_RandomScale = 0.5f;
            /// <summary>
            /// The Seed multiplier for this Rule.
            /// </summary>
            public int m_SeedMultiplier = 1;
            /// <summary>
                ...
                if (RuleMatches(rule, position, tilemap, ref transform))
                {
                    switch (rule.m_Output)
                    {
                        ...
                        case TilingRule.OutputSprite.Animation:
                            ...
                            break;
                        case TilingRule.OutputSprite.RandomPerlin:
                            int perlinIndex = Mathf.Clamp(Mathf.FloorToInt(GetPerlinValue(position, rule.m_PerlinScale, 100000f) * rule.m_Sprites.Length), 0, rule.m_Sprites.Length - 1);
                            tileData.sprite = rule.m_Sprites[perlinIndex];
                            if (rule.m_RandomTransform != TilingRule.Transform.Fixed)
                                transform = ApplyRandomTransform(rule.m_RandomTransform, transform, rule.m_PerlinScale, position);
                            break;
                        case TilingRule.OutputSprite.Random:
                            var seededRandom = new System.Random(position.GetHashCode() * rule.m_SeedMultiplier * rule.m_SeedMultiplier);
                            if (rule.m_RandomScale * 100 > seededRandom.Next(0, 100))
                            {
                                int randomIndex = seededRandom.Next(0, rule.m_Sprites.Length);
                                tileData.sprite = rule.m_Sprites[randomIndex];
                            }
                            else
                            {
                                tileData.sprite = rule.m_Sprites[0];
