    ...
    public class RuleTileEditor : Editor
    {
        ...
        public float GetElementHeight(RuleTile.TilingRuleOutput rule)
        {
            ...
            switch (rule.m_Output)
            {
                case RuleTile.TilingRule.OutputSprite.Random:
                    ...
                    break;
                case RuleTile.TilingRule.OutputSprite.RandomPerlin:
                    inspectorHeight = k_DefaultElementHeight + k_SingleLineHeight * (rule.m_Sprites.Length + 3) + k_PaddingBetweenRules;
                    break;
        ...
        public void RuleInspectorOnGUI(Rect rect, RuleTile.TilingRuleOutput tilingRule)
        {
            ...
            if (tilingRule.m_Output == RuleTile.TilingRule.OutputSprite.Random)
            {
                ...
                tilingRule.m_RandomScale = EditorGUI.Slider(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_RandomScale, 0f, 1f);
                y += k_SingleLineHeight;
                GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Seed");
                tilingRule.m_SeedMultiplier = EditorGUI.IntSlider(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_SeedMultiplier, 1, 100);
                y += k_SingleLineHeight;
            }
            
            if (tilingRule.m_Output == RuleTile.TilingRule.OutputSprite.RandomPerlin)
            {
                GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Noise");
